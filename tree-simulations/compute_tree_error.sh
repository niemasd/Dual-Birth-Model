...
for dir in param*; do
    ...
    for i in $(seq -w 1 20); do
        ...
        echo -n "Tree Error RF = (FN+FP)/2: " >> $dir/trees_inferred_fasttree/$i.inferred.fasttree.support.tre.stats
        echo $(echo -n '(' && echo -n `compareTrees.missingBranch $dir/trees_true_simulated/$i.tre $dir/trees_inferred_fasttree/$i.inferred.fasttree.support.tre | cut -d' ' -f3` && echo -n ' + ' && echo -n `compareTrees.missingBranch $dir/trees_inferred_fasttree/$i.inferred.fasttree.support.tre $dir/trees_true_simulated/$i.tre | cut -d' ' -f3` && echo -n ') / 2') | bc -l >> $dir/trees_inferred_fasttree/$i.inferred.fasttree.support.tre.stats
        echo " done"
