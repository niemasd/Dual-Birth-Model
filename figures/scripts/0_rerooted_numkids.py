...
if len(sys.argv) < 2:
    ...
    exit(-1)
y = sorted([int(i.strip()) for i in open(sys.argv[1]).read().strip().splitlines()])
x = [i for i in range(len(y))]

# create histogram
fig = plt.figure()
plt.plot(x,y)
plt.plot([-1000,8000],[444,444],linestyle='--',color='#000000')
plt.xlim(0,len(x)+1)
...
sns.plt.show()
fig.savefig(sys.argv[1] + '_rerooted_numkids.pdf', format='pdf', bbox_inches='tight')
plt.close()
