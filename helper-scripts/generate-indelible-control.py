...
USAGE_MESSAGE = '''
USAGE:   python generate-indelible-control.py <directory> <length> <replicates> <gtr_freqs> <gtr_rates>
...
EXAMPLE: python generate-indelible-control.py ~/myTrees 300 1 0.2922,0.2319,0.2401,0.2358 0.8896,2.9860,0.8858,1.0657,3.8775,1.0000
'''
# imports
import gzip,os,sys

# generate control file and output as string (which can be written to file)
def generateControl(directory, length, reps, pA, pC, pG, pT, pAC, pAG, pAT, pCG, pCT, pGT):
...
[MODEL] GTRalu      // model name
    ...
    for name in trees:
        control += ('    [' + name + ' GTRalu ' + str(length) + ']\n')

    # evolve trees
    control += ("\n/* Evolve Trees */\n[EVOLVE] pAlu " + str(reps) + " AluOUT // output sequences as AluOUT.fas")
...
if __name__ == '__main__':
    ...
    try:
        control = generateControl(directory,length,reps,pA,pC,pG,pT,pAC,pAG,pAT,pCG,pCT,pGT)
