...
python AluSimulator.py $la $lb 6 $rep|nw_topology -L -|sed -e "s/I//g" -e "s/,)/,d)/g" -e "s/(,/(d,/g" |tee >(sort|uniq -c|sort -k2 > dis_${la}_${lb}_${rep}) | (nw_order -|sort|uniq -c|sort -k2 > unorddis_${la}_${lb}_${rep})
