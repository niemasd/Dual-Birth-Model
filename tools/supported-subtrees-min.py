               ...
               node.m = node.child_nodes()[0].m + node.child_nodes()[1].m
                ...
                for child in node.child_nodes():
                    if child.c == 0:
                        ...
                        node.m += 0 if ext == -1 or child.n == 1  else max(ext*child.n,child.m)
                node.m = max (node.m, 1)
                #node.c = (node.c+0.0)/nsum*(node.n-cleaves) if nsum > 0 else 0
            #if node.c != node.m:
        print("%d %d %d\n%s" %(node.n,node.c,node.m,tree.as_string(schema='newick',suppress_leaf_taxon_labels=True,suppress_leaf_node_labels =True,suppress_internal_taxon_labels=True,suppress_internal_node_labels=True,suppress_annotations=True,suppress_edge_lengths=True)))
